...
char buf[MAX_PATH];
	...
	file.filename = buf;
	file.quickCheck = 0;
	SendMessage(plugin.hwndParent,WM_WA_IPC,(WPARAM)&file,IPC_GET_BASIC_FILE_INFO);*/
...
SendMessage(plugin.hwndParent,WM_WA_IPC,(WPARAM)&fileInfo, IPC_GET_BASIC_FILE_INFO);
...
// passing in wp = -1 will just open the prefs to the last opened page
